...
function updateCart(e) {
  ...
  // Update cat Item count
  cartCount.innerText = cartItemCount.toString();
}

// filter product by check boxes and search input

function filterProducts() {
  // Get search term
  const searchTerm = searchInput.value.trim().toLowerCase();
  // Gey checked categories
  const checkedCategories = Array.from(checkboxes).filter((check) => check.checked)
  .map((check) => check.id);
